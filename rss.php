...
<?php foreach ($rows as $row):
    ...
    if (!empty($row['has_image'])) {
        ...
        if (preg_match('/(?:youtube\.com\/watch\?.*v=|youtu\.be\/|youtube\.com\/embed\/)([a-zA-Z0-9_-]{11})/', $url, $m)) {
            $desc .= '<iframe width="' . $embedSize[0] . '" height="' . $embedSize[1] . '" src="https://www.youtube.com/embed/' . $m[1] . '" frameborder="0" allowfullscreen></iframe>';
        } elseif (preg_match('/vimeo\.com\/(\d+)/', $url, $m)) {
            $desc .= '<iframe src="https://player.vimeo.com/video/' . $m[1] . '" width="' . $embedSize[0] . '" height="' . $embedSize[1] . '" frameborder="0" allowfullscreen></iframe>';
        } else {
